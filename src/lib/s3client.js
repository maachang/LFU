...
// 機能を利用して最小限のプログラムでS3バケットのI/Oを行う.
// ※ s3restApi.js を s3client.jsに統合.
///////////////////////////////////////////////
(function() {
'use strict'
    ...
    if(keyOnly == true) {
        while(true) {
            ...
            // 取得件数を超えてる場合.
                ...
                // 次の内容が存在.
                response.header[NEXT_MARKER_NAME] = "true";
                // 処理終了.
                return ret;
            }
            // Key名だけ取得.
        ...
        // 件数が未満なので、次の内容は存在しない.
        response.header[NEXT_MARKER_NAME] = "false";
        return ret;
    }
    while(true) {
        // コンテンツ条件を取得.
...
const listObject = async function(
    ...
    if(typeof(options.delimiter) == "string") {
        urlParams["delimiter"] = httpsClient.encodeURIToPath(
            options.delimiter);
    }
