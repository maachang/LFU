...
Lambdaの新しい機能 `関数URL` を利用しているので、URLが「AWS側で割り当てられたもの」になるので、指定ドメインのURLとはならない.<br>
一方で、Lambdaの関数URLで `固定のURL` が発行されるので、これを「展開」すれば普通に「Webアプリ利用」できるので、社内システムで利用するレベルなら、この問題は特に問題がないものかと言える。
